...
export default function Filter({
  ...
  const handleApply = () => {
    onFilterChange?.({ search, startDate, endDate }); // ✅ send values to parent
    setIsFilter(false);
  };
    ...
    <div className="flex gap-3 ml-auto mt-2 relative">
      ...
      {isFilter && (
        <div className="w-[330px] px-5 py-4 absolute z-40 top-14 shadow-md right-0 h-[316px] bg-[#FFFFFF] rounded-[14px]">
          ...
          <div className="mt-3">
            <label className="text-[14px] font-[500] text-[#000000] ">
              Start Date
            </label>
            <input
              type="date"
          ...
          <div className="flex gap-3 items-center mt-3">
            <button
              onClick={handleClear}
              className="bg-[#F0F0F0] w-[140px] text-[#6A6A6A] font-[600] text-[12px] rounded-[8px] p-3"
            >
              Clear
            </button>
            <Button
